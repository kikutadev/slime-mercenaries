...
import { captureEnemyRigRestPose, applyEnemySecondaryPose, resolveEnemyRigParts } from './rig';
...
describe('Great Mushroom boss slam', () => {
  it('uses a boss-only squat, lift, hard contact and stable recovery', () => {
    const anticipation = getGreatMushroomAttackMotion(0.38);
    const airborne = getGreatMushroomAttackMotion(0.50);
    const contact = getGreatMushroomAttackMotion(ENEMY_MOTION_THRESHOLDS.bossContactU);
    const recovery = getGreatMushroomAttackMotion(1);

    expect(Math.abs(anticipation.travel)).toBeLessThan(0.01);
    expect(anticipation.scaleY).toBeLessThan(0.90);
    expect(airborne.jump).toBeGreaterThan(0.07);
    expect(contact.jump).toBeLessThan(0.001);
    expect(contact.scaleX).toBeGreaterThan(1.12);
    expect(contact.scaleY).toBeLessThan(0.84);
    expect(contact.wobbleZ).toBe(0);
    expect(recovery.travel).toBeCloseTo(0, 5);
    expect(recovery.jump).toBeCloseTo(0, 5);
  });
});