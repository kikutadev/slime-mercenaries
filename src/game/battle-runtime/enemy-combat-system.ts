...
import { getBossApproachPresentation, getEnemyApproachEntryPose } from '../battle-approach';
...
import { enemyTargetPosition, facePoint, findNearest } from './unit-presentation';
import type { AllyUnit, BattleSnapshot, EnemyUnit } from './types';

type DamageSource = 'melee' | 'projectile' | 'enemy';

export interface BattleEnemyCombatDependencies {
  effects: BattleEffectsSystem;
  getLivingAllies: () => AllyUnit[];
  updateDefeat: (enemy: EnemyUnit, now: number) => void;
  applyDamage: (
...
export class BattleEnemyCombatSystem {
  ...
  constructor(private readonly deps: BattleEnemyCombatDependencies) {}
  ...
  updateApproachIdle(
    ...
    const bossPresentation = enemy.scaleClass === 'boss'
      ? getBossApproachPresentation(now - phaseStartedAt)
      : null;
    ...
    const target = findNearest(enemy, this.deps.getLivingAllies());
    if (target) facePoint(enemy, enemyTargetPosition(target, phase));
  ...
  update(enemy: EnemyUnit, now: number, phase: BattleSnapshot['phase']): void {
    if (enemy.state === 'defeat' || enemy.state === 'dead') {
      this.deps.updateDefeat(enemy, now);
      return;
    }
    ...
    const target = enemy.attackTarget?.alive
      ? enemy.attackTarget
      : findNearest(enemy, this.deps.getLivingAllies());
    if (!target) return;
    ...
    const targetPosition = enemyTargetPosition(target, phase);
    ...
    if (distance > enemy.attackRange) {
      ...
      if (target.behaviorId === 'sword-melee' && phase === 'combat') {
  ...
  private updateAttack(enemy: EnemyUnit, now: number): void {
    ...
    if (!enemy.attackHitApplied && u >= enemy.motionProfile.contactU) {
      ...
      if (attackVfx !== undefined) {
        ...
        this.deps.effects.createImpact(
          impactPosition,
          attackVfx.impactColor,
          attackVfx.impactSize,
          0.34,
        );
        this.deps.startCameraShake(
          attackVfx.cameraShakeDuration,
          attackVfx.cameraShakeAmplitude,
        );
      }
      if (enemy.motionProfile.projectile) {
        this.deps.effects.fireEnemyProjectile(enemy, target);
      } else {
        this.deps.applyDamage(target, enemy.attackDamage, 'enemy', enemy.root.position);
      }
    }
