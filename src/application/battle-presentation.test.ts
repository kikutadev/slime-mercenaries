...
describe('battle presentation handoff', () => {
  it('keeps the current encounter while the authoritative simulation advances', () => {
    expect(canAdoptBattleSceneModel(model('wave-1'), model('wave-2'), false)).toBe(false);
    expect(canAdoptBattleSceneModel(model('wave-1'), model('wave-2'), false)).toBe(false);
  });

  it('hands off to the latest real encounter after the visible result', () => {
    expect(canAdoptBattleSceneModel(model('wave-1'), model('wave-3'), true)).toBe(true);
  });

  it('never replaces a real encounter with an empty content boundary', () => {
    expect(canAdoptBattleSceneModel(model('wave-1'), model('none', false), true)).toBe(false);
  });

  it('accepts visual-only roster changes within the same encounter', () => {
    expect(canAdoptBattleSceneModel(model('wave-1', true, 'a'), model('wave-1', true, 'b'), false)).toBe(true);
  });
});
