...
Combat role: fast cleave / line DPS

## 1. Design intent

This file defines the content-specific model and motion direction for **Blademaster Slime**. Product-level rules in `docs/specs/art-direction.md`, `docs/specs/combat.md`, and `docs/specs/evolution-roster.md` remain authoritative.

## 2. Model / silhouette

Long blade and scarf, expert silhouette, almost no armor clutter.
...
Quiet controlled idle; scarf is the main secondary motion.

### Move / bounce

Low, quick, efficient hops.

### Basic attack

Compress sharply, dash through the target line, stop behind enemies, delayed slash trace resolves.

### Skill / signature behavior

Piercing dash-through arc damages aligned enemies.
...
Signature must read from motion path and delayed cut, not particle count.
