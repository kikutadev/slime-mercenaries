...
Classification: Sword / Fusion Form (non-Codex)  
Combat role: frontline area cleave

## 1. Design intent

This file defines the content-specific model and motion direction for **Greatsword Slime**. Product-level rules in `docs/specs/art-direction.md`, `docs/specs/combat.md`, and `docs/specs/evolution-roster.md` remain authoritative.

## 2. Model / silhouette

Same slime body size as Sword; dramatically broader greatsword; stronger without becoming a larger slime.
...
Deeper weapon-weight lean, but no body-scale growth.

### Move / bounce

Sword-family bounce with stronger weapon lag.

### Basic attack

Fast half-turn horizontal sweep; damaging edge remains nearly horizontal; never a slow full spin.

### Skill / signature behavior

Wide bounded frontal cleave that can hit nearby enemies.
...
Existing `greatsword-slime.glb`; current fast sweep is the reference and must remain a fusion form, not a promotion tier.
